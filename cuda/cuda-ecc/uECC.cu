...
__global__ void uECC_verify_kernel(const uint8_t **public_keys,
    ...
    out[i] = uECC_verify(public_keys[i],
                message_hashes[i],
                hash_sizes[i],
                signatures[i],
                curves[i]);
...
int main() {
    ...
    cudaMalloc(&public_keys, sizeof(*public_keys) * num_threads);
    const uint8_t **message_hashes;
    cudaMalloc(&message_hashes, sizeof(*message_hashes) * num_threads);
    unsigned * hash_sizes;
    cudaMalloc(&hash_sizes, sizeof(*hash_sizes) * num_threads);
    const uint8_t ** signatures;
    cudaMalloc(&signatures, sizeof(*signatures) * num_threads);
    uECC_Curve * curves;
    cudaMalloc(&curves, sizeof(uECC_Curve) * num_threads);
    int * out;
    cudaMalloc(&out, sizeof(int) * num_threads);
 
    uECC_verify_kernel<<<block_size, num_threads>>>(public_keys, message_hashes, hash_sizes, signatures, curves, out);
}
